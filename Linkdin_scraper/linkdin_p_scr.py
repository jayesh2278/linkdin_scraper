...
class LinkedinScraper(object):
    ...
    #     print("results____________________________________________________",results)    
    #     return results
    def parse_links(self):
        soup = BeautifulSoup(self.all_htmls, 'html.parser')
    ...
    def save_to_csv(self, links, profiles):
        data = list(zip(links, profiles))
        with open('linkedin_profiles.csv', 'w', newline='', encoding='utf-8') as csvfile:
            csv_writer = csv.writer(csvfile)
            csv_writer.writerow(['Profile Link', 'Profile Name'])
